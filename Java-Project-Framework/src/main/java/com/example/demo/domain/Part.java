...
public abstract class Part implements Serializable {
    ...
    public String getInventoryValidationMessage() {
        if (min > max) {
            return "Minimum inventory cannot be greater than maximum.";
        }
        if (inv < min) {
            return "Inventory is less than the minimum allowed.";
        }
        if (inv > max) {
            return "Inventory is greater than the maximum allowed.";
        }
        return null;
    }
